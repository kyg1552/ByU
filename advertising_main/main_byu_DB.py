...
def byu_robot_main():
    ...
    ##### 카메라로부터 이미지 받기 시작 ####
    costomer_face_img = '~/byU_main/advertising_DB/costomer_image/' + cur_time + '_' + cur_place + '.jpg' #이미지를 시간별로 저장
    ...
    if capture_result != False: # 카메라가 정상적으로 동작한 경우
        ...
        for face in faces:
            ...
            #gender_data.append(data['gender']) # 예전 DB 저장 부분
            ...
            ## 새로 작성한 DB 저장 부분
            ###customer_data <- ([gender,age,time,place])
            ###customer_data <- ([data['gender'],string(data['age']),cur_time,cur_place])
            
            #customer_data.append([data['gender'])
            #customer_data.append(string(data['age'])
