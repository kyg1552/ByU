...
class Byu:
    def __init__(self, image_queue, place, playtime, resolution_index):
    ...
    def getImageOcamS(self, mode): # ocam으로 이미지 받기
        self.cur_time = time.strftime('%Y%m%d_%H%M%S') # 현재 연/월/일 시간:분:초
    ...
    def advertising(self,mode): # modify vip 
        if mode == 'day':
            self.getImageOcamS(mode = 'day')
