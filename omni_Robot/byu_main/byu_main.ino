...
int front_detection_distance1 = 20;
int front_detection_distance2 = 10;
int Left_detection_distance = 20;
int Right_detection_distance = 20;
int SideL_detection_distance = 20;
int SideR_detection_distance = 20;
...
float motor_Uturn_speed = 0.0; // U턴 속도(z)
float motor_moveTosideL_speed = 0.0; // +: 좌측으로 이동(y)
float motor_moveTosideR_speed = 0.0; // -: 우측으로 이동(y)

float f_decay_rate = 0.5; //전방 장애물 감지 시 속도 감속률 
float side_ = 0.5; // 좌,우 이동 후 반대로 이동 시 속도 감속률

float forward_speed_max = 100;
float side_speed_max = 150;
float turn_speed_max = 0.5;

int front_delay = 100;
int Uturn_delay = 2000; //ms
int turn_delay = 700;
int movoToside_delay = 300;
...
void Avoidance_Check() {
  ...
  if (distance_Side_L <= SideL_detection_distance) { // 앞이 막혀서 회전 한 후에 좌측에 장애물이 가까이 있을 경우 우측으로 이동하여 거리를 두도록 한다.
    ...
    if (distance_Side_R <= SideR_detection_distance) { // 좌측으로 움직였는데 반대쪽 장애물에 가까워진 경우 반대로 다시 살짝 움직여준다.
      drive_line_body_frame( 0, motor_moveTosideL_speed * side_, 0, movoToside_delay);
    }
  }
    ...
    if (distance_Side_L <= SideL_detection_distance) { // 좌측으로 움직였는데 반대쪽 장애물에 가까워진 경우 반대로 다시 살짝 움직여준다.
      drive_line_body_frame( 0, motor_moveTosideR_speed * side_, 0, movoToside_delay);
    }
  }
}
