/* Omni Motor Control ByU Robot
  ...
  업로드 전에 명령: sudo chmod a+rw /dev/ttyACM0
  (avrdide: ser_open(): can't open device "/dev/ttyACM0": Permission denied 에러날 떄
  터미널 창 열고, sudo chmod a+rw /dev/ttyACM0 치면 해결 
...
#define BODY_RADIUS  110 //mm 로봇 반지름
...
void byu_Cb(const geometry_msgs::Transform& byu_val) // 명령 값 받아서 byu robot 제어
{
  ...
  motor_forward_speed = -byu_val.translation.x;
  motor_moveTosideR_speed = byu_val.translation.y;
  turn_delay = byu_val.translation.z;
  
  motor_turnR_speed = byu_val.rotation.z;
 
  drive_line_body_frame(motor_forward_speed, motor_moveTosideR_speed, motor_turnR_speed,turn_delay);
